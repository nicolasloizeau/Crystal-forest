...
function mockTezosTransactionHash() {
	const randomSequence = Array.from({ length: 49 }, () => BASE58_CHARSET[Math.random() * BASE58_CHARSET.length | 0]).join("");
...
*/
function mockEthereumTransactionHash() {
...
*/
const mockBaseTransactionHash = mockEthereumTransactionHash;
/**
...
*/
function mockTransactionHash(chain) {
...
*/
const isBaseTransactionHashValid = isEthereumTransactionHashValid;
/**
* Validates a Tezos transaction hash.
* @param {string} hash - The Tezos transaction hash to validate.
* @returns {boolean} True if the hash is valid, false otherwise.
*/
function isTezosTransactionHashValid(hash) {
	if (hash.length !== 51) return false;
	if (!hash.startsWith("oo") && !hash.startsWith("op")) return false;
	for (let i = 2; i < hash.length; i++) if (!BASE58_CHARSET.includes(hash[i])) return false;
	return true;
}
/**
...
*/
function getBlockchainFromTransactionHash(hash) {
...
*/
function isTransactionHashValid(address) {
	return [
...
//#endregion
export { getBlockchainFromTransactionHash, isBaseTransactionHashValid, isEthereumTransactionHashValid, isTezosTransactionHashValid, isTransactionHashValid, mockBaseTransactionHash, mockEthereumTransactionHash, mockTezosTransactionHash, mockTransactionHash };
//# sourceMappingURL=hash-DpgZuvgv.js.map